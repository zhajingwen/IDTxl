...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('crypto_network_analysis.log'),
...
class CryptoNetworkAnalyzer:
    ...
    def _default_config(self) -> Dict:
        ...
        return {
            ...
            'time_hours': 168,  # 分析时间窗口（7天）
            'cmi_estimator': 'JidtGaussianCMI',
            'max_lag_sources': 12,  # 12小时滞后
            'min_lag_sources': 1,
            'max_lag_target': 6,
            'tau_sources': 1,
            'tau_target': 1,
            'n_perm_max_stat': 100,  # 减少置换次数以提高速度
            'n_perm_min_stat': 100,
            'n_perm_omnibus': 200,
...
def main():
    ...
    # 生成可视化
    print("📈 正在生成可视化结果...")
    analyzer.visualize_results(results, 'crypto_network_analysis.png')
    
    # 生成报告
    print("📝 正在生成分析报告...")
    report = analyzer.generate_report(results, 'crypto_network_report.md')
    ...
    print("\n📁 输出文件:")
    print("- crypto_network_analysis.png (可视化图表)")
    print("- crypto_network_report.md (分析报告)")
    print("- crypto_network_analysis.log (日志文件)")
